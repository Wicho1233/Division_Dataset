...
from .utils import read_arff_dataset, train_val_test_split, validate_file_extension, get_dataset_info, get_column_types, convert_to_arff, find_stratify_column
        ...
        if form.is_valid():
            try:
                ...
                if not validate_file_extension(dataset_file.name):
                    messages.error(request, 'Solo se aceptan archivos .arff')
                    return render(request, 'upload.html', {'form': form})
                ...
                context = {
                    ...
                    'used_stratify': stratify_column if stratify_column else 'Ninguna (sampling aleatorio)',
